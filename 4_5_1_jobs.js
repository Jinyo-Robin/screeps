        ...
        return (configuration, externals, ai) => { return {
            ...
            grunter: {
                ...
                run: function(creep) {
                    var grunterTasks = Memory.tasks
                        ...
                        .sort((a, b) => {
                            return (b.priority - ai.state.getRange(b.ref.pos, creep.pos)) - (a.priority - ai.state.getRange(a.ref.pos, creep.pos));
                        });
            ...
            attacker: {
                ...
                run: function(creep) {
                    ...
                    enemies = [...enemies, ...creep.room.find(FIND_HOSTILE_STRUCTURES).filter(n => n.structureType != STRUCTURE_CONTROLLER)];
                    if(enemies.length > 0){
                        enemies = enemies.sort((a, b) =>  ai.state.getRange(a.pos, creep.pos) - ai.state.getRange(b.pos, creep.pos));
            ...
            transferrer: {
                ...
                run: function(creep) {
                    var transferrerTasks = Memory.tasks
                        ...
                        .sort((a, b) => (b.priority - ai.state.getRange(b.ref.pos, creep.pos)) - (a.priority - ai.state.getRange(a.ref.pos, creep.pos)));
            ...
            upgrader: {
                ...
                run: function(creep) {
                    ...
                    if(!creep.store[RESOURCE_ENERGY]) {
                        var collectorTasks = Memory.tasks
                            .filter(n => n.job == 'collector')
                            .sort((a, b) => (b.priority - ai.state.getRange(b.ref.pos, creep.pos)*70) - (a.priority - ai.state.getRange(a.ref.pos, creep.pos)*70));
                        if(collectorTasks.length){
                            if(collectorTasks[0].ref.store && collectorTasks[0].ref.store[RESOURCE_ENERGY]){
                                if(creep.withdraw(collectorTasks[0].ref, 'energy') == ERR_NOT_IN_RANGE) {
                                    creep.moveTo(collectorTasks[0].ref, {visualizePathStyle: {stroke: '#ffffff'}});
            ...
            claimer: {
                ...
                run: function(creep) {
                    ...
                    if(!creep.store[RESOURCE_ENERGY]) {
                        var collectorTasks = Memory.tasks
                            .filter(n => n.job == 'collector')
                            .sort((a, b) => (b.priority - ai.state.getRange(b.ref.pos, creep.pos)*200) - (a.priority - ai.state.getRange(a.ref.pos, creep.pos)*200));
                        if(collectorTasks.length){
                            if(collectorTasks[0].ref.store && collectorTasks[0].ref.store[RESOURCE_ENERGY]){
                                if(creep.withdraw(collectorTasks[0].ref, 'energy') == ERR_NOT_IN_RANGE) {
                                    creep.moveTo(collectorTasks[0].ref, {visualizePathStyle: {stroke: '#ffffff'}});
